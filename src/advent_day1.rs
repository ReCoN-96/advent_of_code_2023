pub const CALIBRATION_DOCUMENT: &str = "eighttkbtzjz6nineeight
...
fn calibration_document_lines_to_array(value: &str) -> Vec<&str> {
    value.split_whitespace().collect()
}

fn first_digit_string(s: &str) -> String {
    match s.chars().find(|c: &char| c.is_digit(10)) {
        Some(digit) => digit.to_string(),
        None => "".to_string()
    }
}

fn last_digit_string(s: &str) -> String {
    match s.chars().rev().find(|c| c.is_digit(10)) {
        Some(digit) => digit.to_string(),
        None => "".to_string(),
    }
}
...
pub fn calibration_trubuchet(calibration_lines: Vec<&mut str>) -> i32 {

    let mut calibration_trebuchet = 0;

    for calibration_line in calibration_lines {
        let concatenated = format!("{}{}", first_digit_string(calibration_line), last_digit_string(calibration_line));
        if let Ok(parsed) = concatenated.parse::<i32>(){
            calibration_trebuchet += parsed;
        }
    }
    
    calibration_trebuchet
}

fn replace_string_number_to_digit(a: Vec<&str>) -> Vec<&mut str> {
    let mut replaced_strings = Vec::new();
    ...
    for s in &a {
        ...
        replaced_strings.push(Box::leak(replaced.into_boxed_str()));
    }
...
// pub fn part_one() {
//     println!("{:?}",calibration_trubuchet(calibration_document_lines_to_array(CALIBRATION_DOCUMENT)))
// }

pub fn part_two() {
    println!("{:?}", calibration_trubuchet(replace_string_number_to_digit(calibration_document_lines_to_array(CALIBRATION_DOCUMENT))))
}
